...
as D97 does for packages. The global list is the one setup that the machine's
own oku belongs to, so no new setting is needed. The install scripts run before
