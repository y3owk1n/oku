...
- B178 [2] `sync` and `update` install 16 packages at once, or the number in
  `OKU_PARALLEL`, and build one package at a time. `outdated` looks up the
  versions of the same number of packages at once. A package the store holds
...
- B380 [3] `oku du` says that a cloned app or font takes less disk than its
  size. It says so only where a file of the store can become a clone next to
  the apps oku placed.
...
- B369 [3] A generation whose packages are the same as those of the generation
  it replaces uses the same links into the store. Its `oku.lock`, when
